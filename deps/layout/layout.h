...
#else
typedef struct lay_vec4 { int16_t x[4]; } lay_vec4; // __attribute__ ((__vector_size__ (8), aligned(2)));
typedef struct lay_vec2 { int16_t x[2]; } lay_vec2; // __attribute__ ((__vector_size__ (4), aligned(2)));
#endif // LAY_FLOAT
...
{
    lay_item_t *pitem = lay_get_item(ctx, item);
    pitem->size.x[0] = width;
    pitem->size.x[1] = height;
        ...
        lay_context *ctx, lay_id item, int dim)
{
    ...
    while (child != LAY_INVALID_ID) {
        ...
        lay_scalar child_size = rect.x[dim] + rect.x[2 + dim] + pchild->margins.x[wdim];
        ...
        lay_context *ctx, lay_id item, int dim)
{
    ...
    while (child != LAY_INVALID_ID) {
        lay_item_t *pchild = lay_get_item(ctx, child);
        lay_vec4 rect = ctx->rects[child];
        need_size += rect.x[dim] + rect.x[2 + dim] + pchild->margins.x[wdim];
        child = pchild->next_sibling;
    }
    return need_size;
}
...
static void lay_calc_size(lay_context *ctx, lay_id item, int dim)
{
    ...
    ctx->rects[item].x[2 + dim] = cal_size;
}
            ...
            lay_context *ctx, lay_id item, int dim, bool wrap)
{
    ...
    lay_scalar space = rect.x[2 + dim];

    float max_x2 = (float)(rect.x[dim] + space);
    ...
    while (start_child != LAY_INVALID_ID) {
        ...
        while (child != LAY_INVALID_ID) {
            ...
            if ((flags & LAY_HFILL) == LAY_HFILL) {
                ++count;
                extend += child_rect.x[dim] + child_margins.x[wdim];
            } else {
                if ((fflags & LAY_ITEM_HFIXED) != LAY_ITEM_HFIXED)
                    ++squeezed_count;
                extend += child_rect.x[dim] + child_rect.x[2 + dim] + child_margins.x[wdim];
            }
        ...
        while (child != end_child) {
            ...
            x += (float)child_rect.x[dim] + extra_margin;
            if ((flags & LAY_HFILL) == LAY_HFILL) // grow
                x1 = x + filler;
            else if ((fflags & LAY_ITEM_HFIXED) == LAY_ITEM_HFIXED)
                x1 = x + (float)child_rect.x[2 + dim];
            else // squeeze
                x1 = x + lay_float_max(0.0f, (float)child_rect.x[2 + dim] + eater);

            ix0 = (lay_scalar)x;
            if (wrap)
                ix1 = (lay_scalar)lay_float_min(max_x2 - (float)child_margins.x[wdim], x1);
            else
                ix1 = (lay_scalar)x1;
            child_rect.x[dim] = ix0; // pos
            child_rect.x[dim + 2] = ix1 - ix0; // size
            ctx->rects[child] = child_rect;
            x = x1 + (float)child_margins.x[wdim];
            child = pchild->next_sibling;
            extra_margin = spacer;
        }
...
void lay_arrange_overlay(lay_context *ctx, lay_id item, int dim)
{
    ...
    const lay_scalar offset = rect.x[dim];
    const lay_scalar space = rect.x[2 + dim];
    ...
    while (child != LAY_INVALID_ID) {
        ...
        case LAY_HCENTER:
            child_rect.x[dim] += (space - child_rect.x[2 + dim]) / 2 - child_margins.x[wdim];
            break;
        case LAY_RIGHT:
            child_rect.x[dim] += space - child_rect.x[2 + dim] - child_margins.x[dim] - child_margins.x[wdim];
            break;
        case LAY_HFILL:
            child_rect.x[2 + dim] = lay_scalar_max(0, space - child_rect.x[dim] - child_margins.x[wdim]);
            break;
        default:
            break;
        }

        child_rect.x[dim] += offset;
        ctx->rects[child] = child_rect;
        child = pchild->next_sibling;
    }
}
        ...
        lay_scalar offset, lay_scalar space)
{
    ...
    while (item != end_item) {
        ...
        lay_vec4 rect = ctx->rects[item];
        lay_scalar min_size = lay_scalar_max(0, space - rect.x[dim] - margins.x[wdim]);
        switch (b_flags & LAY_HFILL) {
            case LAY_HCENTER:
                rect.x[2 + dim] = lay_scalar_min(rect.x[2 + dim], min_size);
                rect.x[dim] += (space - rect.x[2 + dim]) / 2 - margins.x[wdim];
                break;
            case LAY_RIGHT:
                rect.x[2 + dim] = lay_scalar_min(rect.x[2 + dim], min_size);
                rect.x[dim] = space - rect.x[2 + dim] - margins.x[wdim];
                break;
            case LAY_HFILL:
                rect.x[2 + dim] = min_size;
                break;
            default:
                rect.x[2 + dim] = lay_scalar_min(rect.x[2 + dim], min_size);
                break;
        ...
        lay_context *ctx, lay_id item, int dim)
{
    const int wdim = dim + 2;
    lay_item_t *pitem = lay_get_item(ctx, item);
    lay_scalar offset = ctx->rects[item].x[dim];
    lay_scalar need_size = 0;
    lay_id child = pitem->first_child;
...
static void lay_arrange(lay_context *ctx, lay_id item, int dim)
{
    ...
    case LAY_COLUMN | LAY_WRAP:
        if (dim != 0) {
            ...
            ctx->rects[item].x[2 + 0] = offset - ctx->rects[item].x[0];
        }
        break;
    case LAY_ROW | LAY_WRAP:
        if (dim == 0)
